...
import { HistoricalSyncMonitor, SYNC_PRESETS } from '../historical-sync-config';
...
describe('Historical Sync - Sanity Checks', () => {
  ...
  describe('Basic Connectivity and Setup', () => {
    ...
    it('should initialize ledger successfully', async () => {
      ...
      console.log(
        `✓ Service Version: ${serviceVersion.name} v${serviceVersion.version}`
      );
    });
  ...
  describe('Pagination Configuration', () => {
    it('should have valid pagination config', () => {
      ...
      expect(config.defaultBatchSize).toBe(
        SYNC_PRESETS.CONSERVATIVE.pagination.defaultBatchSize
      );
  ...
  describe('Basic Transaction Fetching', () => {
    it('should fetch transactions from a small block range', async () => {
      ...
      if (transactions.length > 0) {
        ...
        console.log(
          `✓ Found ${transactions.length} transactions in ${batchSize} blocks`
        );
        console.log(`✓ First Transaction: #${firstTx.transactionNumber}`);
      } else {
        console.log(
          `✓ No transactions found in block range ${deploymentBlock}-${deploymentBlock +
            batchSize}`
        );
      }
    });
    ...
    it('should get first 10 anchor events in order', async () => {
      ...
      if (sortedTransactions.length > 0) {
        ...
        console.log(
          `✓ Retrieved ${sortedTransactions.length} sequential transactions`
        );
        console.log(
  ...
  describe('Contract Information', () => {
    ...
    it('should detect contract deployment block', async () => {
      ...
      expect(Math.abs(detectedBlock - deploymentBlock)).toBeLessThanOrEqual(
        tolerance
      );
    });
  });
  ...
  describe('Enhanced Read Method', () => {
    it('should handle read() with no parameters gracefully', async () => {
      ...
      console.log(
        `✓ No-parameter read returned ${result.transactions.length} transactions`
      );
    ...
    it('should handle read() with transaction number', async () => {
      ...
      if (result.transactions.length > 0) {
        ...
        console.log(
          `✓ Found ${result.transactions.length} transactions after #0`
        );
        console.log(
    ...
    it.skip('should handle read() with non-existent transaction number', async () => {
      ...
      console.log(
        `✓ No transactions found after #${highTransactionNumber} (expected)`
      );
    });
...
describe('Historical Sync - Integration Tests', () => {
  ...
  describe('Observer Initialization', () => {
    it('should create observer with historical sync config', () => {
      ...
      console.log(
        `✓ Observer created with sync state: ${JSON.stringify(syncState)}`
      );
    });
  ...
  describe('Sync State Management', () => {
    ...
    it('should detect when caught up (no historical sync needed)', async () => {
      ...
      mockTransactionStore.getLastTransaction.mockReturnValue(
        recentTransaction
      );

      // This would require additional logic in the real implementation
      console.log(
        `✓ Would detect caught-up state for recent transaction #${recentTransaction.transactionNumber}`
      );
    });
  });
  ...
  describe('Configuration Validation', () => {
    it('should validate historical sync config', () => {
      ...
      invalidConfigs.forEach((config, index) => {
        console.log(
          `✓ Testing invalid config ${index + 1}: ${JSON.stringify(config)}`
        );
    ...
    it('should provide reasonable defaults', () => {
      ...
      console.log(
        `✓ Default config is reasonable: ${JSON.stringify(defaultConfig)}`
      );
    });
  });
});
...
describe('Performance Tests', () => {
  ...
  it('should fetch 1000 blocks within reasonable time', async () => {
    ...
    console.log(
      `✓ Fetched ${transactions.length} transactions from ${batchSize} blocks in ${duration}ms`
    );
  ...
  it('should handle concurrent requests', async () => {
    ...
    const promises = [
      ...
      ledger._getTransactions(
        deploymentBlock + batchSize,
        deploymentBlock + 2 * batchSize
      ),
      ledger._getTransactions(
        deploymentBlock + 2 * batchSize,
        deploymentBlock + 3 * batchSize
      ),
    ];
    ...
    console.log(
      `✓ Concurrent fetch: ${totalTransactions} transactions in ${duration}ms`
    );
    console.log(
      `✓ Throughput: ${(totalTransactions / (duration / 1000)).toFixed(
        2
      )} tx/sec`
    );
